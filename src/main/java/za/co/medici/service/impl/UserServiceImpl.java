...
public class UserServiceImpl implements UserService {
    ...
    public User createUser(User user, String usernameHeader) throws UserException {
        ...
        User existinfUser = this.findUserByEmailAndDeletedDateNull(user.getEmail());
        if (!this.checkUtil.isEmpty(existinfUser)) {
            ...
            throw new UserException(String.format("User with Email %s already exists.", user.getEmail()), 400);
        }
    ...
    public User updateUser(UseDto useDto, String usernameHeader) throws UserException {
        ...
        User existinfUser = this.userRepository.findById(useDto.getId()).orElse(null);
        if (this.checkUtil.isEmpty(existinfUser)) {
        ...
        existinfUser = this.userRepository.findUserByEmailAndDeletedDateNull(useDto.getEmail());
        if (!this.checkUtil.isEmpty(existinfUser)
                && !this.checkUtil.notEqual(existinfUser.getEmail(), useDto.getEmail())
                && this.checkUtil.notEqual(existinfUser.getId(), useDto.getId())) {
